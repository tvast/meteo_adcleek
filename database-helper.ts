...
// Replace this with your full SQL string for city insertion
const POP_CITY =
...
export async function initializeDatabase() {
  const db = await open({
    filename: "./local.db", // Use a real .db file, not a .ts script!
  ...
  // Check if city table is already populated
  const { count } = await db.get<{ count: number }>(`SELECT COUNT(*) as count FROM city`);
  if (count === 0) {
    await db.exec(POP_CITY);
    console.log("✅ City data inserted.");
  } else {
    console.log("ℹ️ City table already populated.");
  }
